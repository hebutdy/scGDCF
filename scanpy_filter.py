...
# results_file = 'C:/Users/scRNA-seq data/worm_neuron_cell.h5'
# data_mat = h5py.File(results_file, "r+")
# x = np.array(data_mat['X'])
# y = np.array(data_mat['Y'])
#x = pd.read_csv('E:/scRNA-seq data/worm.csv')
x = pd.read_csv('E:/Bladder_dge.csv')
x = x.loc[:, x.columns[:]].to_numpy()
adata = sc.AnnData(x)
# 添加序列号（index）
...
# 这一句会让基因表达值为负值
#adata.write_h5ad(r'C:/Users/scRNA-seq data/worm_new.h5')
X = adata.X
X_raw = adata.raw.X
print(X)
x = np.array(X)
